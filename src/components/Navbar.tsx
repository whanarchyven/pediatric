...
const Navbar = ({isInteravtive}:navbarInterface) => {
    ...
    return (
        <div className={classList('sm:px-[70px] px-[20px] fixed z-[999]  top-0 left-0 py-1 sm:py-[35px] w-full grid sm:grid-cols-12 grid-cols-6 gap-4 items-center transition-all duration-300',scrolled>120||pathname.split('/')[1]=='account'||pathname.split('/')[1]=='events'||pathname.split('/')[1]=='news'?'navbar-gradient bg-opacity-80 backdrop-blur-sm':'bg-green sm:bg-transparent')}>
